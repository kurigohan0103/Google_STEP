...
class HashTable:
    ...
    def resize(self, new_size):
        new_size = self.get_oddNumber(new_size)
        new_bukets = [] * new_size
        old_bukets = self.buckets
        new_count = 0
        for buket in old_bukets:
            item = buket
            while item is not None:
                buket_index = calculate_hash(item.key) % new_size
                if new_bukets[buket_index] is None:
                    new_bukets[buket_index] = item
                else:
                    concurrent = new_bukets[buket_index]
                    while concurrent.next is not None:
                        concurrent = concurrent.next
                    concurrent.next = item
                next_item = item.next
                item.next = None
                item = next_item
                new_count += 1
        self.buckets = new_bukets
        self.item_count = new_count


    def get_oddNumber(self, num):
        if num % 2 != 0:
            return num
        else:
            return num + 1
